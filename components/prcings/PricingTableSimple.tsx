      ...
      <div className='h-full'>
        <div className='max-w-xs mx-auto'>
          <div className='relative flex flex-col h-full bg-black rounded-lg shadow-lg p-5'>
            ...
            <ul className='text-indigo-200 text-sm space-y-2 flex-grow mb-6'>
              {features.map((feature, index) => (
                <li key={index} className='flex items-center'>
                  <svg
                    className='w-3 h-3 fill-current text-white mr-3 flex-shrink-0'
                    viewBox='0 0 12 12'
            ...
            <a
              href={buttonLink}
              className='font-semibold text-sm inline-flex items-center justify-center px-3 py-2 border border-transparent rounded leading-5 shadow transition duration-150 ease-in-out w-full bg-cta hover:bg-green-500 text-black focus:outline-none focus-visible:ring-2'
            >
              {buttonText}
            </a>
          </div>
